...
*   `DestinationPattern`:
    *   `Any`
    *   `Exact([u8; 20])`: A specific destination address.
...
⚠️ **Important**: Before computing the policy hash, you must first fill the first policy line in `examples/policy.json` (id: 1) with the address of the deployed ZKGuard module. 
This allows updating policies, groups and allowlists in the module by defining a policy for it. If you do **NOT** define such a policy, no updates can happen, and therefore, there is no way to update the policy, groups, or allowlist anymore.
