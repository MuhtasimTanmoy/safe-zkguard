...
use zkguard_core::{
    ...
    SignerPattern as CoreSignerPattern, TxType as CoreTxType,
};
...
impl From<JsonDestinationPattern> for CoreDestinationPattern {
    fn from(v: JsonDestinationPattern) -> Self {
        match v {
            ...
            JsonDestinationPattern::Exact(s) => {
                let mut addr = [0u8; 20];
                let s = s.strip_prefix("0x").unwrap_or(&s);
                let bytes = hex::decode(s).expect("invalid hex in destination Exact address");
                addr.copy_from_slice(&bytes);
                CoreDestinationPattern::Exact(addr)
            }
...
impl From<JsonPolicyLine> for CorePolicyLine {
    fn from(val: JsonPolicyLine) -> Self {
        CorePolicyLine {
            ...
            amount_max: val
                .amount_max
                .map(|s| s.parse::<u128>().expect("invalid amount_max")),
...
fn main() -> Result<()> {
    ...
    #[derive(Parser, Debug)]
    struct Args {
        /// JSON file with policy lines (optional)
        #[clap(long)]
        policy: Option<String>,
        /// JSON file with groups (optional)
        #[clap(long)]
        groups: Option<String>,
        /// JSON file with allow-lists (optional)
        #[clap(long)]
        allow: Option<String>,
    }
    ...
    if let Some(policy_path) = args.policy.as_deref() {
        let json_policy: Vec<JsonPolicyLine> = read_json_file(policy_path)?;
    ...
    // Groups (optional)
    if let Some(groups_path) = args.groups.as_deref() {
        let json_groups: HashMap<String, Vec<String>> = read_json_file(groups_path)?;
        ...
        for (k, v) in json_groups.into_iter() {
            ...
            for s in v {
                ...
                let bytes = hex::decode(&s)
                    .with_context(|| format!("invalid hex address for key {}", k))?;
                let mut arr = [0u8; 20];
                arr.copy_from_slice(&bytes);
                addrs.push(arr);
            }
    ...
    // Allowlists (optional)
    if let Some(allow_path) = args.allow.as_deref() {
        let json_allow: HashMap<String, Vec<String>> = read_json_file(allow_path)?;
