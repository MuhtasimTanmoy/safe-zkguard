...
library NickAddress {
    ...
    function createRandomAddress(bytes32 seed) internal pure returns (address addr) {
        ...
        unchecked {
            for (uint256 i = 0; i < 1024; ++i) {
                ...
                uint256 rNum = (uint256(keccak256(abi.encodePacked(seed, i, "r"))) % (N - 1)) + 1;
                uint256 sNum = (uint256(keccak256(abi.encodePacked(seed, i, "s"))) % (HALF_N - 1)) + 1;
                uint8 v = uint8(uint256(keccak256(abi.encodePacked(seed, i, "v"))) & 1) + 27;
